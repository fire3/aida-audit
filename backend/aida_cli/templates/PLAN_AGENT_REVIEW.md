...
- 调用 `audit_plan_list(plan_type='audit_plan')` 查看宏观计划
- 调用 `audit_plan_list(plan_type='agent_plan')` 查看所有已有任务及其状态
...
### 步骤 2：分析决策
基于审查结果，做出以下决策：
- **情况 A：存在 pending/in_progress 状态的任务**
  - 不创建新任务，现有任务正在执行中
- **情况 B：上一个任务发现了可疑点/漏洞**
...
- `audit_plan_list` (查看所有计划)
- `audit_get_notes` / `audit_get_findings` (查看发现和笔记)
