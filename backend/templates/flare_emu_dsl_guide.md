...
- **定义变量**：通过 `alloc` 的 `var` 字段或 `call` 的 `return_var` 字段定义变量。这些变量存储在 DSL 解释器的内存中，用于在步骤间传递数据（如内存地址、句柄等）。
...
# 调用函数 (支持变量参数和返回值)
$ret = call my_function($str, 123)

# 带 Hook 的调用
call my_function() {
    hook 0x401000 {
        action: read_reg reg.rax -> $temp
    }
}

# 日志输出 (支持变量替换)
log "Function returned: $ret"
...
为了方便调试复杂的仿真场景，DSL 提供了以下增强功能：

### 1. 自动日志与返回值
- 每次 `call` 结束后，系统会自动在日志中打印函数的返回值（如 `Function returned: 0x...`）。
- 使用 `log "message"` 指令可以在脚本执行过程中输出调试信息，支持变量替换（如 `$var`）。

### 2. 函数调用跟踪
开启 `trace_calls` 选项后，模拟器会尝试检测并记录执行流中的函数调用（CALL 指令）。
```dsl
option trace_calls = true
```
这有助于理解函数内部的控制流。
...
### 3. 栈完整性检查
在 `call` 指令执行前后，系统会自动检查栈指针（SP）的变化。
- 如果 SP 在函数返回后没有恢复到预期位置（例如发生了栈溢出或调用约定不匹配），系统会输出 **WARNING** 日志：
  `Stack pointer mismatch: start=..., end=...`
这对于检测栈溢出漏洞或错误的调用约定非常有用。
