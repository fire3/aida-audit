...
EXPORT int test_strlen(const char* str) {
    int len = 0;
    while (str[len] != '\0') {
        len++;
    }
...
EXPORT void test_xor_crypt(char* buf, int len, char key) {
    for (int i = 0; i < len; i++) {
        buf[i] ^= key;
    }
}
...
// Returns: sum
EXPORT int test_many_args(int a, int b, int c, int d, int e, int f) {
    return a + b + c + d + e + f;
}
...
EXPORT int test_complex_path(int val) {
    int result = 0;
    if (val > 100) {
        result = val * 2;
    } else if (val < 0) {
