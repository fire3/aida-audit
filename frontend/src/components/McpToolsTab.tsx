...
import type { McpTool, McpToolProperty } from '../api/client';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from './ui/card';
import { Code } from 'lucide-react';

export function McpToolsTab() {
  const { data: mcpTools, isLoading: isMcpToolsLoading } = useQuery<McpTool[]>({
    queryKey: ['mcpTools'],
              ...
              <CardContent className="flex-1">
                ...
                <div className="bg-muted p-3 rounded-md text-xs font-mono overflow-x-auto">
                  <ul className="list-disc list-inside space-y-1">
                    {Object.entries(tool.inputSchema.properties ?? {}).map(([param, details]: [string, McpToolProperty]) => (
                      <li key={param}>
                        <span className="font-bold text-foreground">{param}</span>
                        <span className="text-muted-foreground ml-1">({details.type ?? 'unknown'})</span>
